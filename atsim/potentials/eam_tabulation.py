from .pair_tabulation import (
    PairTabulation_AbstractBase,
    Excel_PairTabulation,
    _r_value_iterator,
)
...
class _EAMTabulationAbstractbase(PairTabulation_AbstractBase):
    ...
    def __init__(
        self, potentials, eam_potentials, cutoff, nr, cutoff_rho, nrho, target
    ):
        """
...
class SetFL_EAMTabulation(_EAMTabulationAbstractbase):
    ...
    def __init__(self, potentials, eam_potentials, cutoff, nr, cutoff_rho, nrho):
        ...
        :params nrho: Number of points to be used when discretising density range during EAM tabulation"""
        super().__init__(
            potentials, eam_potentials, cutoff, nr, cutoff_rho, nrho, "setfl"
        )
...
class SetFL_FS_EAMTabulation(_EAMTabulationAbstractbase):
    ...
    def __init__(self, potentials, eam_potentials, cutoff, nr, cutoff_rho, nrho):
        ...
        :params nrho: Number of points to be used when discretising density range during EAM tabulation"""
        super().__init__(
            potentials, eam_potentials, cutoff, nr, cutoff_rho, nrho, "setfl_fs"
        )
...
class TABEAM_EAMTabulation(_EAMTabulationAbstractbase):
    ...
    def __init__(self, potentials, eam_potentials, cutoff, nr, cutoff_rho, nrho):
        ...
        :params nrho: Number of points to be used when discretising density range during EAM tabulation"""
        super().__init__(
            potentials, eam_potentials, cutoff, nr, cutoff_rho, nrho, "DL_POLY_EAM"
        )
...
class TABEAM_FinnisSinclair_EAMTabulation(_EAMTabulationAbstractbase):
    ...
    def __init__(self, potentials, eam_potentials, cutoff, nr, cutoff_rho, nrho):
        ...
        :params nrho: Number of points to be used when discretising density range during EAM tabulation"""
        super().__init__(
            potentials, eam_potentials, cutoff, nr, cutoff_rho, nrho, "DL_POLY_EAM_fs"
        )
...
class ADP_EAMTabulation(SetFL_EAMTabulation):
    ...
    def __init__(
        ...
        :params cutoff_rho: Density cutoff.
        :params nrho: Number of points to be used when discretising density range during EAM tabulation"""
        super().__init__(potentials, eam_potentials, cutoff, nr, cutoff_rho, nrho)
        self._target = "eam_adp"
...
class Excel_EAMTabulation(_EAMTabulationAbstractbase):
    ...
    def __init__(self, potentials, eam_potentials, cutoff, nr, cutoff_rho, nrho):
        ...
        super().__init__(
            ...
            cutoff_rho,
            nrho,
            self._excel_tab_name,
        )
        self._inner_tabulation = None

    def _build_workbook(self):
        self._inner_tabulation = Excel_PairTabulation(
            self.potentials, self.cutoff, self.nr
        )
    ...
    def _add_eam_density(self, wb):
        ...
        self._inner_tabulation._populate_worksheet(
            ws, "r", _r_value_iterator(self), column_heads, pot_dict
        )
    ...
    def _add_eam_embed(self, wb):
        ...
        column_heads = sorted(pot_dict.keys())
        self._inner_tabulation._populate_worksheet(
            ws, "rho", _rho_value_iterator(self), column_heads, pot_dict
        )
...
class Excel_ADP_EAMTabulation(Excel_EAMTabulation):
    """Class for dumping ADP EAM model into a spreadsheet"""

    _excel_tab_name = "excel_eam_adp"
    ...
    def _add_sheets(self, wb):
        super()._add_sheets(wb)
        self._add_dipole(wb)
        self._add_quadrupole(wb)

    def _add_dipole(self, wb):
        self._inner_tabulation._add_pair_like_worksheet(wb, "EAM-ADP-Dipole", self.dipole_potentials)

    def _add_quadrupole(self, wb):
        self._inner_tabulation._add_pair_like_worksheet(wb, "EAM-ADP-Quadrupole", self.quadrupole_potentials)
