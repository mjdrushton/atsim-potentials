...
from atomsscripts.lammps import writeEAMTable
from atomsscripts.potentials import Potential, EAMPotential, buck, TableReader
...
class RunLAMMPSEAMTableTestCase(TempfileTestCase):
  ...
  def testFluorite_NoPair(self):
    ...
    try:
      ...
      with open('eam.fs', 'wb') as outfile:
        writeEAMTable.writeSetFLFinnisSinclair(
            nrho, drho,
            nr, dr,
            eampots,
            [],
  ...
  def testWriteFuncflPair(self):
    ...
    try:
      ...
      from atomsscripts.potentials import writeFuncFL
  ...
  def testFuncFL_Example1(self):
    ...
    try:
      ...
      from atomsscripts.potentials import Potential, EAMPotential
  ...
  def testWriteSetFLPair(self):
    ...
    try:
      ...
      from atomsscripts.potentials import writeSetFL
  ...
  def testWriteSetFLDensity(self):
    ...
    try:
      ...
      eamPotentials = [
        EAMPotential("Cu", 29, 63.55, embed, fdict["dens_Cu"]),
        EAMPotential("Al", 13, 26.98, embed, fdict["dens_Al"])]
      ...
      shutil.copyfile(os.path.join(_getResourceDirectory(), "setfl_triplet.lmpstruct"), os.path.join(self.tempdir,"structure.lmpstruct"))
      dens_Cu = fdict["dens_Cu"]
      dens_Al = fdict["dens_Al"]
  ...
  def testCompareZhouTabulation(self):
    ...
    try:
      shutil.copyfile(os.path.join(_getResourceDirectory(), "calc_energy.lmpin"), os.path.join(self.tempdir,"calc_energy.lmpin"))
      shutil.copyfile(os.path.join(_getResourceDirectory(), "setfl_triplet.lmpstruct"), os.path.join(self.tempdir,"structure.lmpstruct"))
      shutil.copyfile(os.path.join(_getResourceDirectory(), "Zhou_AlCu.setfl"), os.path.join(self.tempdir, "Zhou_AlCu.setfl"))
...
class LAMMPSWriteEAMTableTestCase(unittest.TestCase):
  ...
  def testWriteSetFLFromParameters(self):
    """Test that lammps.writeEAMTable.writeSetFL() can generate the Al_zhou.eam.alloy file from LAMMPS distribution"""
  ...
  def testWriteSetFLFinnisSinclair(self):
    """Test that lammps.writeEAMTable.writeSetFLFinnisSinclair() (suitable for use with pair_style eam/fs) can re-create AlFe_mm.eam.fs file from lammps distribution"""
