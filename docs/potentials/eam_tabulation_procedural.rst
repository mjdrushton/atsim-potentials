...
The example uses the :func:`.writeSetFLFinnisSinclair` function to produce files supported by the LAMMPS ``pair_style eam/fs`` command. 

The potential model and definition of potential objects is detailed in :ref:`eam_example_3b_obj` which uses a tabulation class but is otherwise very similar to this example. Having defined the list of :class:`.EAMPotential` instances the :func:`.writeSetFLFinnisSinclair` function is called, in this case writing the data to ``Mendelev_Al_Fe.eam.fs`` in the current directory:
