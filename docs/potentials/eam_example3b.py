...
from atsim.potentials.eam_tabulation import TABEAM_FinnisSinclair_EAMTabulation
...
def ppfuncAlAl(r):
    funcs = [
        ((0.0, 1.6), zerowrap(lambda r: (2433.5591473227/r) * (0.1818 * math.exp(-22.713109144730 * r) + 0.5099 *
                                                               math.exp(-6.6883008584622 * r) + 0.2802 * math.exp(-2.8597223982536*r) + 0.02817 * math.exp(-1.4309258761180*r)))),
        ((1.6, 2.25), lambda r: math.exp(6.0801330531321 - 2.3092752322555 *
                                         r + 0.042696494305190*r**2 - 0.07952189194038*r**3)),
        ((2.25, 3.2), lambda r:  17.222548257633*(3.2 - r)**4 - 13.838795389103*(3.2 - r)**5 +
            26.724085544227*(3.2 - r)**6 - 4.8730831082596*(3.2 - r)**7 + 0.26111775221382*(3.2 - r)**8),
        ((2.25, 4.8), lambda r: -1.8864362756631*(4.8 - r)**4 + 2.4323070821980*(4.8 - r)**5 -
            4.0022263154653*(4.8 - r)**6 + 1.3937173764119*(4.8 - r)**7 - 0.31993486318965*(4.8 - r)**8),
    ...
    vals = [func(r) for ((lowcut, highcut), func)
            in funcs if lowcut <= r < highcut]
    return sum(vals)


def ppfuncAlFe(r):
    funcs = [
        ((0.0, 1.2), zerowrap(lambda r: (4867.1182946454/r) * (0.1818 * math.exp(-25.834107666296*r) + 0.5099 *
                                                               math.exp(-7.6073373918597*r) + 0.2802*math.exp(-3.2526756183596*r) + 0.02817*math.exp(-1.6275487829767*r)))),
        ((1.2, 2.2), lambda r: math.exp(6.6167846784367-1.5208197629514 *
                                        r - 0.73055022396300*r**2 - 0.038792724942647*r**3)),
        ((2.2, 3.2), lambda r: -4.1487019439249*(3.2 - r)**4 + 5.6697481153271*(3.2 - r)**5 -
            1.7835153896441*(3.2 - r)**6 - 3.3886912738827*(3.2 - r)**7 + 1.9720627768230*(3.2 - r)**8),
...
def ppfuncFeFe(r):
    funcs = [
        ((0.0, 1.0), zerowrap(lambda r: (9734.2365892908/r)*(0.1818*math.exp(-28.616724320005*r) + 0.5099 *
                                                             math.exp(-8.4267310396064*r) + 0.2802*math.exp(-3.6030244464156*r) + 0.02817*math.exp(-1.8028536321603*r)))),
        ((1.0, 2.05), lambda r: math.exp(7.4122709384068 -
                                         0.64180690713367*r - 2.6043547961722*r**2 + 0.62625393931230*r**3)),
